function [res_bord, res_mod, list_residu] = SchwarzAitkenSVD_2c(model1, model2, y0, nb_iter_schwarz, eps, max_iter)
    ...
    while(i<max_iter && list_residu(end)>eps && not(bool_convergence))
        i=i+1;
        [mat_all_iter,cell_all_iter_bord,res_mod_gauche,res_mod_droit,res_schwarz,bool_convergence]=iter_solve_2c(model1,model2,nb_iter_schwarz,y0,eps);
        disp(size(res_schwarz));

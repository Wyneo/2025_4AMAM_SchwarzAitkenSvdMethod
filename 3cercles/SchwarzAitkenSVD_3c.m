function [res_bord, res_mod, list_residu] = SchwarzAitkenSVD_3c(model1, model2, model3, y0, nb_iter_schwarz, eps, max_iter)
    ...
    while(i<max_iter && list_residu(end)>eps && not(bool_convergence))
        ...
        res_bord_c1=acc_aitkenSVD(cell_all_iter_bord{1}, 1e-2); 
        res_bord_c2=acc_aitkenSVD(cell_all_iter_bord{2}, 1e-2);
        res_bord_c3=acc_aitkenSVD(cell_all_iter_bord{3}, 1e-2);
        y_prec=y0;
        y0=res_bord_c1;
